...
public:
  ...
  typedef pair<pair<typename GRAPH_PATTERN::VERTEX_T, typename GRAPH_PATTERN::VERTEX_T >, typename GRAPH_PATTERN::EDGE_T > MAP_EDGE_T;
  ...
  int parse_next_trans(ifstream& infile, pat_fam<GRAPH_PATTERN>& freq_pats, storage_manager<GRAPH_PATTERN, 
                       VAT, ALLOC, SM_T>& vat_hmap, FREQ_MAP& fm) {
    std::string word;
	...
	map_update(fm, local_fm);
	  ...
	  map_update(fm, local_fm);
        ...
        while(count<num_items) {
          ...
          switch(count) {
            case 0: vid=atoi(word.c_str()); break;
            case 1:

...
class db_reader
{
 public:
  ...
  typedef map<pair<pair<typename PATTERN::VERTEX_T, typename PATTERN::VERTEX_T >, typename PATTERN::EDGE_T >, int> FREQ_MAP;
   ...
   */
  template<class SM_T>
  void get_length_one(pat_fam<PATTERN>& freq_pats, storage_manager<PATTERN, VAT, ALLOC, SM_T>& vat_hmap, const int& minsup, FREQ_MAP& fm) {

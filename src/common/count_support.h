...
public:
  ...
  void count(PATTERN* const& p1, PATTERN* const& p2, PATTERN** const& cand_pats, 
             const int& minsup, const int& num, const bool& isfwd, const pair<int, int>& ids) {
    ...
    // of each cand_pat
    cand_vats= _strg_mgr.intersect(p1, p2, cand_sups, cand_pats, isfwd, ids, minsup);
  ...
  VAT* get_vat(PATTERN* const& p) {
    return _strg_mgr.get_vat(p);
  }

  // Prints the tids in which this pattern occurs.
  void print_tids(PATTERN* const& p) {
    _strg_mgr.print_tids(p);
  }
